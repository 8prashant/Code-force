...
There is a bookshelf which can fit n
 books. The i
-th position of bookshelf is ai=1
...
: move the book at index i
 to i+1
 for all l≤i≤r
. This move can be done only if r+1≤n
...
: move the book at index i
 to i−1
 for all l≤i≤r
. This move can be done only if l−1≥1
...
The first line of the test case contains one integer n
 (1≤n≤50
) — the number of places on a bookshelf. The second line of the test case contains n
 integers a1,a2,…,an
 (0≤ai≤1
), where ai
 is 1
...
void check(int n){
    ...
    int head=0,tail=0;
    for(int i=0;i<n;i++){
        if(st1[i]==0 && head==tail){
            head+=1;
            tail+=1;
        }
        if(st1[i]==1 && head==tail){
            tail+=1;
        }
        if(st1[i]==0 && head!=tail){
            tail+=1;
